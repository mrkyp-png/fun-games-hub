(function (root) {
  ...
  function create({ layer }) {
    ...
    // 대기 애니메이션(사용자 지정, 기존 4동작 잽 시도는 철회) — 잠깐 조용하면(화장실 등)
    // 한 번씩 좌우 글러브가 가운데서 만난다(라운드 인트로 인사 제스처 meet() 재사용).
    const IDLE_DELAY = 2.5, IDLE_MEET_GAP = 3.4;
    let idleT = 0;
    function update(dt) {
      ...
      idleT += dt;
      if (idleT > IDLE_DELAY) {
        idleT = IDLE_DELAY - IDLE_MEET_GAP; // 다음 만남까지 간격만 남기고 되감기(반복 사이클)
        meet();
      }
    }
