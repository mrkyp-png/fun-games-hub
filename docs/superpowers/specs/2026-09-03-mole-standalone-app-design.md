...
> - **온보딩**: 앱 최초 실행 시 대화 화면 앞에 메이커 1회 강제. 그 뒤 대화 인트로(첫 방문) 정상 표시.
...
**한 줄 요약:** 홈화면을 카톡 "더보기" 스타일 다크 화면으로 새로 만들고, **핵심 콘텐츠 = 사람두더지 메이커**(내 사진 → 얼굴 → 두더지 머리 합성 → 그 얼굴로 게임)를 넣는다. 난이도는 하수/고수/전설. 하트·코인·광고 메타를 얹는다. 캐릭터(두더지 오빠/하마)·카톡 대화 인트로는 뺀다.
